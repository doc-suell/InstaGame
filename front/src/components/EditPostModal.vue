...
const submitForm = async () => {
  ...
  try {
    ...
    formData.append('new_description', newDescription.value);

    const response = await axios.post(`http://localhost/instaGame/controller/postController.php?id=${postId}`, formData, {
      headers: {
  ...
  <div class="post-container" v-if="isOpenEdit">
    ...
    <div class="form-container-post">
      ...
      <form @submit.prevent="submitForm" enctype="multipart/form-data" class="form-items post-form">
        <textarea v-model="newDescription" name="description" placeholder="Write a caption..."></textarea>
        <div>
